/**
 * NaWeHub — Web Home Page
 * --------------------------------------
 * Location: app/(web)/page.tsx
 *
 * Rebuilt on the same design system as the Next Big Idea and
 * vetted-entrepreneurs pages: globals.css tokens (no hardcoded zinc/emerald/
 * blue/indigo/rose), the `container mx-auto px-4` width convention, and the
 * Fraunces/Inter/JetBrains Mono fonts your root layout now provides
 * globally.
 *
 * What changed structurally, and why:
 *
 * 1. Dropped the Header/AppHeader/Footer imports and renders. Your
 *    app/(web)/layout.tsx now wraps every page in this folder with Header
 *    and Footer — rendering them again here would duplicate both.
 *
 * 2. Dropped the `isVisible` useState/useEffect fade-in choreography (every
 *    section had `translate-y-8 opacity-0` flipping to visible on mount).
 *    Since it only fired once on mount regardless of scroll position, it
 *    was effectively just a one-time page-load flash, not a meaningful
 *    scroll reveal — removed for simplicity and to avoid a layout flash on
 *    slow connections. This file no longer needs 'use client' as a result;
 *    ContactUsSection and ContactForm still are, since they hold real state.
 *
 * 3. Hero background now matches the Next Big Idea / vetted-entrepreneurs
 *    hero exactly (`from-primary-50 to-muted/40`, with a `dark:` variant) —
 *    previously this page used its own grid-pattern + blurred gradient-orb
 *    treatment, which read as a different design system from the rest of
 *    the site. All hero text now uses theme tokens instead of zinc-900/
 *    zinc-300/etc.
 *
 * 4. The "Two Platforms" cards are now color-coded to match the actual
 *    pages they link to: the Business Hub card uses `primary` (green,
 *    matches the brand's growth color), and the Next Big Idea card uses
 *    `accent` (orange) — the same orange that's the primary CTA color on
 *    the actual Next Big Idea page. Previously these were arbitrary
 *    blue/indigo and emerald/teal gradients unrelated to either page.
 *
 * 5. The Business Registration CTA reuses the fixed warm-near-black
 *    "neutral-900" brand block from Header/Footer/the Next Big Idea CTA
 *    banner, plus a small inline ClothBorder (the diamond-trim divider used
 *    elsewhere) — ties this moment into the same recurring signature
 *    instead of a one-off zinc-900 gradient with a noise texture.
 *
 * 6. <Benefits /> is left exactly as-is — that component wasn't part of
 *    what you shared, so I didn't touch its internals. It may need the same
 *    token pass if it's still on hardcoded colors.
 */
