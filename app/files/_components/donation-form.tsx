...
import { PaymentStatusModal } from "@/app/files/_components/payment-status-modal"
...
export function DonationForm({ onSubmitAction, onCancelAction }: DonationFormProps) {
  ...
  const handleSubmit = () => {
    startTransition(async () => {
      try {
        ...
        // Open payment modal immediately — SSE will drive state from here
        if (response?.contributionId) {
          setActiveContributionId(response.contributionId)
          setShowPaymentModal(true)
        } else {
          // Fallback if no contributionId returned
          toast({
  ...
  function getPayMethod(val: string) {
    if (val === "Momo") setPaymentOptions((prev) => ({ ...prev, method: "Mobile Money" }))
    else if (val === "Bank") setPaymentOptions((prev) => ({ ...prev, method: "Bank Transfer" }))
    else if (val === "Card") setPaymentOptions((prev) => ({ ...prev, method: "Credit/Debit Card" }))
  }

  function getPayProvider(val: string) {
    if (val === "m17") setPaymentOptions((prev) => ({ ...prev, provider: "Orange Money" }))
    else if (val === "m18") setPaymentOptions((prev) => ({ ...prev, provider: "Afri Money" }))
  }
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Minimum donation: {formatCurrencyLocal(5, formData.currency)}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
              {[5, 10, 15, 20, 25, 30].map((amount) => (
                <Button
                  ...
                  className="text-xs"
                >
                  {formatCurrencyLocal(amount, formData.currency)}
                </Button>
              ))}
            </div>
            ...
            {formData.amount >= 5 && (
              ...
              >
                ...
                <p className="text-sm text-green-700 dark:text-green-300">
                  Your donation of {formatCurrencyLocal(formData.amount, formData.currency)} will make a real
              ...
              {formData.paymentMethod && (
                ...
                >
                  ...
                  >
                    ...
                    <SelectContent>
                      {paymentProviders[formData.paymentMethod as keyof typeof paymentProviders]?.map(
                        (provider) => (
                          <SelectItem
                            key={provider.id}
                            value={provider.id}
                            disabled={provider.id !== "m17"}
                          >
                            {provider.name}
                          </SelectItem>
                        )
              ...
              {!isExistingContributory && (
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-sm space-y-1">
                    <p>
                      <span className="font-medium">Name:</span> {formData.profile.firstName}{" "}
                      {formData.profile.lastName}
                    </p>
                    <p>
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-sm space-y-1">
                  <p>
                    ...
                    {formatCurrencyLocal(formData.amount, formData.currency)}
                  </p>
                  <p>
                    <span className="font-medium">Target:</span>{" "}
                    {formData.target === "District"
                      ? selectedDistrict?.name + " District"
                      : formData.targetValue + ` (${selectedDistrict?.name} District)`}
                  </p>
                  <p>
                    <span className="font-medium">Payment:</span> {paymentOptions.method} via{" "}
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {!isExistingContributory && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Every Leone counts towards district development projects that create lasting impact. Join
      ...
      >
        {isExistingContributory ? (
          <div>
            <p className="text-lg">
              Hello {formData.profile.firstName} {formData.profile.lastName}!
            </p>
            <p className="text-muted-foreground text-sm">
              We're delighted to welcome you back. Great people like you continue to make a difference
              in the development of their districts and chiefdoms. So far, your total contribution is
              SLE {data?.totalContributions} — thank you for your generosity!
            </p>
          </div>
        ) : (
          isNewContribConfirm &&
          currentStep > 1 && (
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Welcome to Love Your District! Great people like you always make a lasting impact by
