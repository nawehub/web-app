...
import {
  Heart, Plus, Calendar, TrendingUp, Users, MapPin,
  Trophy, Star, ArrowUpRight, DollarSign, Building2, Sparkles,
} from "lucide-react"
...
import { LiveBadge } from "@/app/lyd/_components/live-badge"
import { useContributionStream } from "@/app/lyd/hooks/use-contribution-stream"
...
export default function LYDPage() {
  ...
  const { contributors, rankings, isLive, hasError } = useContributionStream()
  const isMobile = useIsMobile()

  const totalContributions = rankings.reduce((sum, r) => sum + (r.totalContributions?.amount ?? 0), 0)
  const totalContributors  = rankings.reduce((sum, r) => sum + r.totalContributors, 0)
  ...
  if (showDonationForm) return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-zinc-950">
      ...
      <div className={`flex-1 container mx-auto px-4 py-8 ${isMobile ? "mt-20" : "mt-16"}`}>
        <DonationForm
          onSubmitAction={() => setShowDonationForm(false)}
          onCancelAction={() => setShowDonationForm(false)}
        />
      </div>
      <Footer />
    </div>
    ...
    <div className="min-h-screen flex flex-col bg-white dark:bg-zinc-950">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        ...
        <div className="container relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-6 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-rose-500/10 border border-rose-500/20">
              <Heart className="w-4 h-4 text-rose-600 dark:text-rose-400" />
              <span className="text-sm font-medium text-rose-700 dark:text-rose-300">Community Impact Program</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-center pt-4">
              <Button size="lg" onClick={() => setShowDonationForm(true)}
                className="bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white font-semibold rounded-xl shadow-lg shadow-rose-500/25 h-12 px-8">
                <Plus className="mr-2 h-5 w-5" />Make a Contribution
              </Button>
              <Button size="lg" variant="outline" onClick={() => setShowDonationHistory(true)}
                ...
                <Calendar className="mr-2 h-5 w-5" />View History
              </Button>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-8 container">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          {[
            { title: "Total Contributions", value: formatCurrency(totalContributions), change: "+12.5%", icon: DollarSign,  gradient: "from-blue-500 to-cyan-500" },
            { title: "Total Contributors",  value: totalContributors?.toLocaleString(), change: "+8.2%",  icon: Users,       gradient: "from-emerald-500 to-teal-500" },
            { title: "Active Districts",    value: allDistricts.length, label: "All 16 districts participating", icon: MapPin, gradient: "from-violet-500 to-purple-500" },
            { title: "Funded Projects",     value: 127, change: "+15.3%", icon: Building2, gradient: "from-orange-500 to-amber-500" },
          ].map((stat, i) => {
            const Icon = stat.icon
            return (
              <Card key={i} className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-lg shadow-zinc-200/50 dark:shadow-zinc-900/50 hover:shadow-xl transition-all duration-300">
                ...
                <CardContent>
                  <div className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-white">{stat.value}</div>
              ...
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="mt-2">Top performing districts by total contributions</CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    {/* Live indicator updates automatically as SSE fires */}
                    <LiveBadge isLive={isLive} hasError={hasError} />
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                  <AnimatePresence>
                    {rankings.map((district, index) => {
                      const pct = totalContributions > 0
                        ? ((district.totalContributions?.amount ?? 0) / totalContributions) * 100
                        : 0
                      return (
                        <motion.div
                        ...
                        >
                          <div className={`flex items-center justify-center w-8 h-8 rounded-xl font-bold text-sm ${
                            index < 3 ? "bg-gradient-to-br from-yellow-400 to-amber-500 text-white" : "bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300"
                          }`}>{index + 1}</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1.5">
                              <p className="font-medium truncate text-zinc-900 dark:text-white">{district.districtName}</p>
                              <p className="text-sm font-semibold text-zinc-900 dark:text-white whitespace-nowrap ml-4">
                                {formatCurrency(district.totalContributions?.amount ?? 0)}
                              </p>
                            </div>
              ...
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <CardDescription className="mt-2">Leading contributors making the biggest impact</CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    <LiveBadge isLive={isLive} hasError={hasError} />
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                  <AnimatePresence>
                    {contributors.map((c, index) => {
                      const name     = c.anonymous ? "Anonymous Contributor" : `${c.firstName} ${c.lastName}`
                      const initials = c.anonymous ? "AN" : `${c.firstName[0] ?? ""}${c.lastName[0] ?? ""}`.toUpperCase()
                      const label    = c.totalContributionsCount === 1 ? "contribution" : "contributions"
                      return (
                        <motion.div
                          key={c.contributorId}
                          layout
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className={`flex items-center justify-center w-8 h-8 rounded-xl font-bold text-sm ${
                              index < 3 ? "bg-gradient-to-br from-blue-500 to-indigo-600 text-white" : "bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300"
                            }`}>{index + 1}</div>
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-medium truncate text-zinc-900 dark:text-white">{name}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-xs text-zinc-500 dark:text-zinc-400 truncate">
                                  {c.anonymous ? "Private contributor" : c.nationality}
                                </span>
                                <Badge variant="secondary" className="text-[10px] bg-white/70 dark:bg-zinc-800 border border-blue-200 dark:border-blue-800">
                                  {c.totalContributionsCount} {label}
                                </Badge>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-semibold text-blue-600 dark:text-blue-400">
                                {formatCurrency(c.totalContributions?.amount ?? 0)}
                              </p>
                            </div>
                          </div>
                        </motion.div>
          ...
          <Card className="relative overflow-hidden border-0 bg-gradient-to-br from-rose-600 via-pink-600 to-red-600">
            <div className="absolute inset-0 opacity-10">
              <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                <defs><pattern id="heart-grid" width="20" height="20" patternUnits="userSpaceOnUse">
            ...
            <CardContent className="relative py-12 px-6 md:px-12">
              <div className="max-w-2xl mx-auto text-center text-white space-y-6">
                ...
                <h3 className="text-2xl sm:text-3xl font-bold">Make a Difference Today</h3>
                <p className="text-white/80 text-lg">Your contribution, no matter how small, helps build schools, hospitals, roads, and other vital infrastructure that transforms communities across Sierra Leone.</p>
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-center pt-4">
                  <Button size="lg" onClick={() => setShowDonationForm(true)}
                    className="bg-white text-rose-600 hover:bg-zinc-100 font-semibold rounded-xl shadow-xl h-12 px-8">
                    <Heart className="mr-2 h-5 w-5" />Start Contributing Today
                  </Button>
                  <Button size="lg" variant="outline" onClick={() => setShowDonationHistory(true)}
                    className="border-2 border-white/30 text-white hover:text-white hover:bg-white/20 rounded-xl h-12 px-8">
                    <TrendingUp className="mr-2 h-5 w-5" />Track Your Impact
                  </Button>
                </div>
              </div>
            </CardContent>
