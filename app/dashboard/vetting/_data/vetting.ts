...
/** Trust score at or above this threshold auto-approves and features on the landing page. */
export const AUTO_FEATURE_SCORE_THRESHOLD = 85;

export function qualifiesForAutoFeature(vettingCase: VettingCase): boolean {
    return (
        vettingCase.status === "approved" &&
        trustOverall(vettingCase.trust) >= AUTO_FEATURE_SCORE_THRESHOLD
    );
}

export function computeAutoFeaturedIds(cases: VettingCase[]): string[] {
    return cases
        .filter(qualifiesForAutoFeature)
        .sort((a, b) => trustOverall(b.trust) - trustOverall(a.trust))
        .map((c) => c.id);
}

/** Auto-approve pending/in-review cases that reach the featured score threshold. */
export function applyAutoVettingRules(cases: VettingCase[]): VettingCase[] {
    return cases.map((c) => {
